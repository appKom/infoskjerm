...
export function EventCard({event}){
  ...
  return(
    <div className="flex flex-col flex-1 bg-white border border-gray-200 rounded-lg shadow">
      <div className={`${event.image ? '' : 'flex justify-center'} w-full h-48 border-b rounded-t-lg border-gray-200`}>
        {event.image ?
          ...
          <OnlineLogo fillColor="#0B5374" />}
      </div>

      <div className='flex flex-col justify-between flex-grow gap-2 px-5 pt-2 pb-3'>
        <div>
          <h5 className="w-full text-2xl font-bold tracking-tight text-gray-900 line-clamp-1">{event.title}</h5>
