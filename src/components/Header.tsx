...
export function Header() {
  ...
  useEffect(() => {
    const timeInterval = setInterval(() => {
      ...
      const currentDateFormatted = moment().format('YYYY-MM-DD HH:mm');
      const lastRefreshTime = localStorage.getItem('lastRefreshTime');
      if (moment().format('HH:mm') === REFRESH_TIME && currentDateFormatted !== lastRefreshTime) {
        localStorage.setItem('lastRefreshTime', currentDateFormatted);
    ...
    const messageInterval = setInterval(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setShowMessage(false), INTERVAL_SECOND);
      }, INTERVAL_SECOND * MESSAGE_TIME_SECONDS);
    }, INTERVAL_SECOND * 60 * MESSAGE_INTERVAL_MINUTES);
  ...
  return (
    <div className='relative h-32 border-b-[1.5px] border-light-grey dark:border-gray-700'>
      {showMessage && (
        <div className='absolute top-0 left-0 flex items-center justify-center w-full h-full text-6xl italic dark:text-white'>
          {MESSAGE_CONTENT}
        </div>
      )}
