...
export const listFiles = async (directory: string) => {
  ...
  const metadataArray = await Promise.all(jsonFiles.map(async file => {
    const metadata = JSON.parse(await fs.promises.readFile(path.join(directory, file), { encoding: 'utf8' }));
    return metadata;
  }));

  // Sort the array so that the most recent files are first
  metadataArray.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
